...
import tabular.core.{DataFactory, Statement, Tabular}
import tabular.list.ListTable
import tabular.spark.RddTable
import tabular.util.RowOrdering

class QueryTest extends TabularCommonTest  with BeforeAndAfterAll{
  ...
  runTest("list test", listTable, data)
  runTest("spark test", rddTable, data)
...
trait TabularCommonTest extends FunSpec with ShouldMatchers{
  import QueryTest._
  def runTest(name: String, table: Tabular[Person], data: Seq[Person]) {
    describe(name) {
...
object QueryTest extends FunSpec with ShouldMatchers {
